...
class MultiRewardNetwork(nn.Module):
    def __init__(self, input_dim, num_stocks, hidden_dim=64,
                 ...
                 # drawdown scaling params
                 rho: float = 1.0,
                 beta_dd_base: float = 1.0,
                 beta_k: float = 1.0,
        ...
        # base drawdown knee and smoothing
        self.dd_kappa = dd_kappa  # baseline ignore drawdowns below this threshold
        self.dd_tau = dd_tau  # Smoothness parameter for SoftPlus

        # risk preference parameter rho in [0,1]. Lower rho => more conservative -> stronger dd penalty
        self.rho = float(rho)
        # beta_dd scaling: beta_dd(rho) = beta_dd_base * (1 + beta_k * (1 - rho))
        self.beta_dd_base = float(beta_dd_base)
        self.beta_k = float(beta_k)
        # kappa knee range for rho mapping
    ...
    def forward(self, state, action, wealth_info=None):
        ...
        if self.use_drawdown and wealth_info is not None:
            ...
            # Compute rho-dependent kappa (knee) and beta scaling
            dd_kappa_rho = self.kappa_max - (self.kappa_max - self.kappa_min) * self.rho

            # Smooth penalty: -SoftPlus((dd - dd_kappa_rho) / tau)
            # SoftPlus(x) = log(1 + exp(x)) ≈ smooth max(0, x)
            dd_scaled = (dd - dd_kappa_rho) / self.dd_tau
            R_dd = -F.softplus(dd_scaled)  # [B, 1]

            # Scale drawdown penalty by beta_dd(rho)
            beta_dd_rho = self.beta_dd_base * (1.0 + self.beta_k * (1.0 - self.rho))
            R_dd = R_dd * beta_dd_rho
...
def model_predict(args, model, test_loader, expert_trajectories=None):
    ...
    # If GA expert trajectories are provided, summarize their selected stocks
    if expert_trajectories is not None and len(expert_trajectories) > 0:
        print("\nGA Expert Selection Summary:")
        # actions are second element in each tuple
        actions = np.array([a for _, a in expert_trajectories])
        # per-trajectory selected indices (may be many); print first few trajectories
        n_sample = min(10, len(actions))
        for i in range(n_sample):
            sel_idx = np.where(actions[i] == 1)[0]
            print(f"  Traj {i+1}: selected {len(sel_idx)} stocks -> indices: {sel_idx.tolist()}")

        # frequency across trajectories
        freq = actions.sum(axis=0)
        topk = min(20, len(freq))
        top_idx = np.argsort(-freq)[:topk]
        print("\n  Top selected stock indices across expert trajectories:")
        for idx in top_idx:
            print(f"    idx {int(idx):4d}: selected in {int(freq[idx])} trajectories ({freq[idx]/len(actions)*100:.1f}%)")
...
def train_model_and_predict(model, args, train_loader, val_loader, test_loader):
    ...
    if not args.multi_reward:
        ...
        # Drawdown scaling parameters (conservative defaults: penalize drawdown strongly)
        # Set rho=0.0 by default to make strategy conservative unless user overrides via CLI
        rho = getattr(args, 'rho', 0.0)
        # Increase base scaling and sensitivity so low rho produces stronger penalties
        beta_dd_base = getattr(args, 'beta_dd_base', 2.0)
        beta_k = getattr(args, 'beta_k', 2.0)
        # Tighter kappa range: smaller knee so more drawdowns are penalized
        kappa_min = getattr(args, 'kappa_min', 0.01)
        kappa_max = getattr(args, 'kappa_max', 0.05)
    ...
    model_predict(args, model, test_loader, expert_trajectories=expert_trajectories)
    return model