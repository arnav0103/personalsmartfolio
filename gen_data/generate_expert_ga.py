...
class StockSelectionGA:
    ...
    def __init__(self,
        ...
        # penalty coefficient applied when a chromosome selects outside the target range
        # higher value => larger penalty for deviating from target range
    ...
    def evaluate_chromosome(self,
        ...
        # Soft constraint: prefer selecting within [min_stocks, max_stocks]
        # Instead of outright rejecting chromosomes outside the range, apply a penalty
        # proportional to how far the selection count deviates from the allowed range.
        ...
        # Weighted fitness (base)
        fitness = sum(
            self.fitness_weights[key] * metrics[key]
            for key in self.fitness_weights.keys()
        )

        # Apply soft penalty if selection count is outside target range
        penalty = 0.0
        if n_selected < min_stocks:
            diff = float(min_stocks - n_selected)
            penalty = self.selection_penalty * (diff / max(1.0, n_stocks))
        elif n_selected > max_stocks:
            diff = float(n_selected - max_stocks)
            penalty = self.selection_penalty * (diff / max(1.0, n_stocks))
    ...
    def initialize_population(self, n_stocks: int) -> List[np.ndarray]:
        ...
        population = []
        # Allow broader initial diversity: sample any feasible number of stocks
        # (1..n_stocks). The soft penalty in evaluate_chromosome will guide
        # selection toward preferred ranges without hard rejection.
        for _ in range(self.population_size):
            n_select = np.random.randint(1, n_stocks + 1)
